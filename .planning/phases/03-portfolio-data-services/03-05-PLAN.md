...
@.planning/phases/03-portfolio-data-services/03-04-SUMMARY.md
...
<task type="auto">
  ...
  <action>
Update all aggregation query builders in portfolio_queries.py to integrate FX conversion using fx_spot table (from Plan 03-04 Task 1).
...
Ensure all portfolio metrics and aggregation endpoints now use consistent FX conversion tied to run's snapshot_id. This completes PORT-07 multi-currency aggregation requirement using FX spots from Plan 03-04.
  </action>
  <verify>
```bash
# Start services
...
# FX spots already ingested in Plan 03-04
...
```
  ...
  <done>FX conversion integrated across all aggregation queries. FX spots tied to market snapshot_id for consistency. Multi-currency positions aggregate correctly to USD. All portfolio metrics use same FX rates from run's snapshot. PORT-07 requirement complete.</done>
</task>
...
- [ ] FX spots stored per market snapshot_id (from Plan 03-04)
...
- FX conversion integrated across all portfolio queries (using Plan 03-04 FX spots)
