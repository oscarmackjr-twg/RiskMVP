...
must_haves:
  truths:
    ...
    - "User can upload FX spots tied to market snapshots via API"
  ...
  artifacts:
    - path: "services/data_ingestion_svc/app/routes/market_feeds.py"
      ...
      exports: ["ingest_yield_curve", "ingest_credit_spreads", "ingest_fx_spots", "ingest_rating_change"]
  ...
  key_links:
    ...
    - from: "FX spot ingestion"
      to: "fx_spot table"
      via: "UPSERT with snapshot_id binding"
      pattern: "ON CONFLICT (pair, snapshot_id) DO UPDATE"
---

<objective>
Implement data ingestion service for market feeds (yield curves, credit spreads, FX spots, ratings) and loan servicing data. Enable data lineage tracking and audit trails for regulatory compliance.

Purpose: Data integration layer for Phase 3; enables historical data versioning, multi-currency support, and traceability.
Output: Working data ingestion service with validation, lineage tracking, FX spot management, and batch status monitoring.
</objective>
...
<task type="auto">
  <name>Task 1: Implement market data feed ingestion with FX spots and lineage</name>
  <files>
services/data_ingestion_svc/app/routes/market_feeds.py
services/common/hash.py
  </files>
  <action>
Replace stub implementations in market_feeds.py with working endpoints for yield curves, credit spreads, FX spots, and ratings ingestion.
...
5. `POST /api/v1/ingestion/market-feeds/fx-spots` - Ingest FX spots (PRIMARY ENDPOINT)
   - Accept FXSpotUpload (snapshot_id, as_of_date, source, spots: List[{pair, spot_rate}])
   - Validate pair format: '{CCY1}/{CCY2}' (e.g., 'EUR/USD', 'GBP/USD')
   - Validate spot_rate > 0
   - UPSERT into fx_spot table (NOT market_data_feed - fx_spot has dedicated table):
     ```sql
     ...
     ```
   - Record lineage with feed_type='FX_SPOT':
     ```sql
     INSERT INTO data_lineage
       (lineage_id, feed_type, feed_id, source_system, source_identifier,
        ingested_at, transformation_chain, quality_checks_passed, metadata_json)
     VALUES (%(lid)s, 'FX_SPOT', %(sid)s, %(src)s, %(src)s,
             ...
             %(meta)s::jsonb);
     ```
     lineage_id = f"fx-{snapshot_id}-{datetime.utcnow().isoformat()}"
     metadata_json = {"pair_count": len(spots), "snapshot_id": snapshot_id}
   - Return FXSpotStatus (snapshot_id, pair_count, ingested_at)
...
6. `GET /api/v1/ingestion/market-feeds/fx-spots/{snapshot_id}` - Get FX spots for snapshot
   ...
   - Return List[FXSpotOut] with pair, spot_rate, as_of_date
...
**CRITICAL:** FX spot ingestion is now a PRIMARY first-class endpoint (endpoints 5-6), not secondary text. This addresses PORT-07 multi-currency aggregation requirement by ensuring FX spots are available and tied to market snapshots.
  </action>
  <verify>
```bash
...
# Test FX spot ingestion (PRIMARY ENDPOINT)
...
# Retrieve FX spots
curl -s http://localhost:8005/api/v1/ingestion/market-feeds/fx-spots/test-snapshot-1 | jq .
...
```
  ...
  <done>Market data feed ingestion working. Yield curves, credit spreads, FX spots, and ratings ingest with UPSERT idempotency. FX spots are first-class endpoints tied to snapshot_id for multi-currency aggregation (PORT-07). Lineage records created for all ingestions. Status endpoint lists recent feeds with validation results.</done>
</task>
...
- [ ] FX spots ingest as first-class endpoint tied to snapshot_id
...
- User can upload market data (yield curves, spreads, FX spots, ratings) via API
- FX spots are first-class endpoints tied to market snapshots for multi-currency support
