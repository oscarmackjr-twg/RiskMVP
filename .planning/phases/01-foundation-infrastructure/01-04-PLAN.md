...
must_haves:
  truths:
    ...
    - "API Gateway routes requests to services via path-based routing"
  ...
  artifacts:
    ...
    - path: "terraform/ecs.tf"
      provides: "ECS Fargate cluster and task definitions"
      min_lines: 50
    ...
    - path: "terraform/api_gateway.tf"
      provides: "API Gateway with path-based routing"
      min_lines: 40
      contains: "resource \"aws_api_gateway_rest_api\""
  key_links:
    ...
    - from: "terraform/api_gateway.tf"
      to: "terraform/ecs.tf"
      via: "service endpoint integration"
      pattern: "integration_uri.*ecs"
---

<objective>
Provision complete AWS infrastructure with Terraform for production deployment.

Purpose: Completes PLAT-04 (Terraform AWS infrastructure). Establishes VPC, ECS Fargate, Aurora PostgreSQL, API Gateway, and ECR. Research confirms Terraform is industry standard in finance for declarative IaC.
...
<task type="auto">
  <name>Task 4: Create Terraform API Gateway for path-based routing</name>
  <files>
terraform/api_gateway.tf
terraform/README.md
  </files>
  <action>
Create API Gateway with path-based routing to ECS services.

**api_gateway.tf:**
Create:
- aws_api_gateway_rest_api (name: iprs-api)
- Resources and methods:
  - /mkt/* → ALB target group for marketdata service (port 8001)
  - /orch/* → ALB target group for orchestrator service (port 8002)
  - /results/* → ALB target group for results service (port 8003)
- Integration: HTTP_PROXY to ALB DNS name
- Deployment: dev stage
- CORS enabled for all methods

Alternative approach (simpler for MVP):
Use ALB path-based routing instead of API Gateway:
- ALB listener rule: /mkt/* → marketdata target group
- ALB listener rule: /orch/* → orchestrator target group
- ALB listener rule: /results/* → results target group

Research suggests: ALB path-based routing is simpler and cheaper for internal API. API Gateway adds features (throttling, API keys, caching) not needed for MVP.

Decision: Use ALB path-based routing. Document in README.md. Defer API Gateway to Phase 2+ if needed.
...
- How to access services (ALB DNS + path prefix)
...
cat terraform/README.md | grep "terraform apply"
  ...
  <done>
API Gateway or ALB path routing configured. README.md documents Terraform workflow. All modules validate.
  </done>
</task>
...
7. ALB or API Gateway configured for path-based routing
8. README.md documents Terraform workflow
</success_criteria>
