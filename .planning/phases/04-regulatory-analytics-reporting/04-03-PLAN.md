...
<task type="auto">
  <name>Task 1: Implement CECL and Basel III route handlers (REG-02, REG-03)</name>
  <files>
    services/regulatory_svc/app/routes/cecl.py
    services/regulatory_svc/app/routes/basel.py
    services/regulatory_svc/app/models.py
...
def get_cecl_staging(portfolio_id: str):
    ...
    # Use stage_classification from compute module
    # Fetch positions with current PD, origination PD, days past due
    # Return stage counts: {stage_1: count, stage_2: count, stage_3: count}
    raise HTTPException(status_code=501, detail="Staging endpoint not yet implemented")
```
...
```
  ...
  <done>CECL and Basel routes implemented with audit trail logging, integrated with compute modules from Plan 02, ready for service startup</done>
</task>
